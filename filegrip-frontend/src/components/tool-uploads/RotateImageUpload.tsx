...
export default function RotateImageUpload({
  toolSlug,
  inputTypes,
  maxFileSizeMb = 25,
}: RotateImageUploadProps) {
  ...
  function handleFile(file: File | null) {
    ...
    if (file.size > maxBytes) {
      setLimitModal({
        ...
        message: `"${file.name}" is bigger than the current upload limit. Compress it first, then rotate it.`,
    ...
    if (!isSupportedImage(file)) {
      setError("Rotate Image only accepts JPG, PNG, and WEBP files.");
      return;
    }
    ...
    <div className="rounded-[2rem] border border-[#E7E5E4] bg-white/90 p-4 shadow-[0_24px_70px_rgba(17,24,39,0.08)] backdrop-blur sm:p-5 dark:border-white/10 dark:bg-white/[0.04] dark:shadow-none">
      <div className="rounded-[1.5rem] border-2 border-dashed border-[#FDBA74] bg-[#FFF7ED] p-5 text-center sm:p-6 dark:border-[#F97316]/50 dark:bg-[#F97316]/10">
        ...
        <p className="mx-auto mt-3 max-w-2xl text-sm leading-6 text-[#78716C] dark:text-white/60">
          Rotate your image left, right, or upside down with a live preview
          before download.
        </p>
      ...
      {selectedFile && (
        <div className="mt-5 grid gap-5 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
          <div className="rounded-[1.5rem] border border-[#E7E5E4] bg-white p-4 dark:border-white/10 dark:bg-white/[0.035]">
            ...
            <div className="mt-5 overflow-hidden rounded-[1.25rem] border border-[#E7E5E4] bg-[#FFF7ED]/70 p-6 dark:border-white/10 dark:bg-[#F97316]/10">
              {previewUrl ? (
                <div className="flex min-h-[430px] items-center justify-center overflow-hidden">
                  <img
                    src={previewUrl}
                    alt={selectedFile.name}
                    className="max-h-[350px] max-w-full rounded-xl object-contain shadow-[0_18px_45px_rgba(17,24,39,0.12)] transition-transform duration-300"
                    style={{
          ...
          <div className="rounded-[1.5rem] border border-[#E7E5E4] bg-white p-4 dark:border-white/10 dark:bg-white/[0.035]">
            ...
            <p className="mt-1 text-xs font-medium text-[#78716C] dark:text-white/45">
              Preview your rotation instantly before processing.
            </p>
            ...
            <div className="mt-5 grid gap-3">
              <button
                ...
                className={`inline-flex items-center justify-center gap-2 rounded-[1.25rem] border px-4 py-4 text-sm font-black transition disabled:cursor-not-allowed disabled:opacity-50 ${
                  rotation === 270
              ...
              <button
                ...
                disabled={isProcessing || Boolean(job)}
                className={`rounded-[1.25rem] border px-4 py-4 text-sm font-black transition disabled:cursor-not-allowed disabled:opacity-50 ${
                  rotation === 180
                    ? "border-[#F97316] bg-[#F97316] text-white"
                    : "border-[#E7E5E4] bg-[#FAFAF9] text-[#57534E] hover:border-[#F97316] hover:text-[#F97316] dark:border-white/10 dark:bg-white/[0.04] dark:text-white/60"
                }`}
            ...
            {isProcessing && (
              <ToolProcessingPanel
                ...
                subtitle="FileGrip is rotating your image and preparing the final download."
              />
            )}
            ...
            {job && (
              <div ref={resultRef}>
                <ToolResultCard
                  ...
                  resetLabel="Rotate another image"
                  onReset={clearFile}
                />
              </div>
            )}
