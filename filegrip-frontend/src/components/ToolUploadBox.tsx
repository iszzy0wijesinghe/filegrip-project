...
export default function ToolUploadBox({
  ...
  async function handleFiles(files: FileList | null) {
    ...
    if (toolSlug === "merge-pdf" && pdfOnlyError) {
  ...
  async function processFiles() {
    ...
    if (toolSlug === "merge-pdf" && selectedFiles.length < 2) {
  ...
  if (isProcessing) {
    return (
      <FileGripLoader label="Merging your PDFs securely..." minTimeMs={2200} />
    );
  }
  ...
  return (
    <div className="rounded-[2rem] border border-[#E7E5E4] bg-white/90 p-4 shadow-[0_24px_70px_rgba(17,24,39,0.08)] backdrop-blur sm:p-5 dark:border-white/10 dark:bg-white/[0.04] dark:shadow-none">
      <div className="rounded-[1.5rem] border-2 border-dashed border-[#FDBA74] bg-[#FFF7ED] p-5 text-center sm:p-7 dark:border-[#F97316]/50 dark:bg-[#F97316]/10">
        <div className="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-2xl bg-[#F97316] text-white shadow-lg shadow-orange-500/25">
          <Upload size={28} />
        </div>
      ...
      {selectedFiles.length > 0 && (
        <div className="mt-5 rounded-[1.5rem] border border-[#E7E5E4] bg-white p-4 dark:border-white/10 dark:bg-white/[0.035]">
          <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
            ...
            <div className="flex gap-2">
              <button
                type="button"
                onClick={() => inputRef.current?.click()}
                className="rounded-full border border-[#E7E5E4] px-4 py-2 text-xs font-black text-[#57534E] transition hover:border-[#F97316] hover:text-[#F97316] dark:border-white/10 dark:text-white/60"
              >
                Add more
              </button>
              ...
              <button
                type="button"
                onClick={clearFiles}
                className="rounded-full border border-[#E7E5E4] px-4 py-2 text-xs font-black text-[#57534E] transition hover:border-red-300 hover:text-red-600 dark:border-white/10 dark:text-white/60 dark:hover:border-red-500/40 dark:hover:text-red-300"
              >
                Clear
              </button>
            </div>
            ...
            {selectedFiles.map((item, index) => (
              ...
              >
                ...
                <div className="mt-3 flex items-start gap-2">
                  <button
                    type="button"
                    className="mt-0.5 cursor-grab rounded-lg p-1 text-[#A8A29E] active:cursor-grabbing dark:text-white/35"
          ...
          <button
            ...
            className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-full bg-[#F97316] px-5 py-3.5 text-sm font-black text-white shadow-[0_16px_35px_rgba(249,115,22,0.22)] transition hover:-translate-y-0.5 hover:bg-[#EA580C]"
          >
            <FileText size={18} />
      ...
      {job && (
        <div className="mt-5 rounded-2xl border border-[#BBF7D0] bg-green-50 p-4 text-left dark:border-green-500/20 dark:bg-green-500/10">
          <div className="flex items-start gap-2 text-sm font-bold text-green-700 dark:text-green-300">
            <CheckCircle2 size={18} className="mt-0.5 shrink-0" />
            <span>{job.message || "File job created successfully."}</span>
          </div>

          {job.download_url && (
            <a
              href={job.download_url}
              className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-full bg-[#111827] px-5 py-3 text-sm font-black text-white transition hover:bg-[#F97316] dark:bg-[#F97316] dark:hover:bg-[#FB923C]"
            >
              <Download size={17} />
              Download merged PDF
            </a>
