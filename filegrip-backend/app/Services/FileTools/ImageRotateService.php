...
class ImageRotateService
{
    ...
    public function rotate(
        ...
        if (! in_array($rotation, [90, 180, 270], true)) {
            throw new RuntimeException('Rotation must be 90, 180, or 270 degrees.');
        }
        ...
        $rotatedImage = imagerotate($sourceImage, $gdRotation, $transparent);

        if (! $rotatedImage) {
            imagedestroy($sourceImage);
            throw new RuntimeException('Could not rotate image.');
        }

        imagepalettetotruecolor($rotatedImage);
        imagealphablending($rotatedImage, true);
        imagesavealpha($rotatedImage, true);

        $this->ensureDirectory($outputPath);
        $this->saveImage($rotatedImage, $outputPath, $outputFormat, $quality);
    ...
    private function saveImage(\GdImage $image, string $outputPath, string $format, int $quality): void
    {
        ...
        if (! $saved || ! file_exists($outputPath) || filesize($outputPath) === 0) {
            throw new RuntimeException('Could not save rotated image.');
        }
    }
